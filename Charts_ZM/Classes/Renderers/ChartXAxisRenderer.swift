...
public class ChartXAxisRenderer: ChartAxisRendererBase
{
    ...
    public var customXAxisValues: [String]?
    ...
    public func drawLabels(context context: CGContext, pos: CGFloat, anchor: CGPoint)
    {
        ...
        // 绘制自定义的X轴刻度
        if let temp = self.customXAxisValues{
            let interval = viewPortHandler.contentWidth / (CGFloat(temp.count) - 1)
            
            for i in 0 ..< temp.count{
                let label = temp[i]
                
                position.x = viewPortHandler.offsetLeft + interval * CGFloat(i)
                position.y = viewPortHandler.contentBottom
...
//                interval = viewPortHandler.offsetLeft + (CGFloat((label! as NSString).floatValue) - previous)/CGFloat((xAxis.values.last! as NSString).floatValue)
//                
//                position.x =  interval
//                position.y = viewPortHandler.contentBottom
//                
//                previous = CGFloat((label! as NSString).floatValue
//)
//                if (viewPortHandler.isInBoundsX(position.x))
//                {
//                    drawLabel(context: context, label: label!, xIndex: i, x: position.x, y: pos, attributes: labelAttrs, constrainedToSize: labelMaxSize, anchor: anchor, angleRadians: labelRotationAngleRadians)
...
//        
//
        ChartUtils.drawMultilineText(context: context, text: label, point: CGPoint(x: x, y: y), attributes: attributes, constrainedToSize: constrainedToSize, anchor: anchor, angleRadians: angleRadians)
    }
