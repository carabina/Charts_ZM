...
public class LineChartRenderer: LineRadarChartRenderer
{
    ...
    private var chartFillDisplayWidth: CGFloat = 0
...
//        let maxx = min(max(minx + 2, dataSet.entryIndex(entry: entryTo) + 1), realCount)
...
//            var curDy: CGFloat = 0.0
            ...
            let prevPrev: ChartDataEntry! = dataSet.entryForIndex(minx)
            var prev: ChartDataEntry! = prevPrev
            var cur: ChartDataEntry! = prev
            let next: ChartDataEntry! = dataSet.entryForIndex(minx + 1)
            ...
            for i in (minx).stride(to: min(size, entryCount-1), by: 1){
                prev = dataSet.entryForIndex(i)
                cur = dataSet.entryForIndex(i+1)

                var prevPoint: CGPoint = CGPointMake(CGFloat(prev.xIndex), CGFloat(prev.value) * phaseY)
                trans.pointValueToPixel(&prevPoint)
                var xval =  (lineData.xVals[i]! as NSString).floatValue
                prevPoint.x = (CGFloat(xval) / lineData.maxXAxisValue) * self.viewPortHandler.contentWidth + self.viewPortHandler.offsetLeft
                
                var curPoint: CGPoint = CGPointMake(CGFloat(cur.xIndex), CGFloat(cur.value) * phaseY)
                trans.pointValueToPixel(&curPoint)
                 xval =  (lineData.xVals[i+1]! as NSString).floatValue
                curPoint.x = (CGFloat(xval) / lineData.maxXAxisValue) * self.viewPortHandler.contentWidth + self.viewPortHandler.offsetLeft
        
                CGPathAddQuadCurveToPoint(cubicPath, nil, prevPoint.x, prevPoint.y, (curPoint.x + prevPoint.x) / 2, (curPoint.y + prevPoint.y) / 2)
                self.chartFillDisplayWidth = curPoint.x;
                
                if(i == entryCount - 2){
                    let last:ChartDataEntry! = dataSet.entryForIndex(entryCount - 1)
                    var lastPoint: CGPoint = CGPointMake(CGFloat(last.xIndex), CGFloat(last.value) * phaseY)
                    trans.pointValueToPixel(&lastPoint)
                    let xval =  (lineData.xVals[entryCount - 1]! as NSString).floatValue
                    lastPoint.x = (CGFloat(xval) / lineData.maxXAxisValue) * self.viewPortHandler.contentWidth + self.viewPortHandler.offsetLeft
                    
                    CGPathAddQuadCurveToPoint(cubicPath, nil, lastPoint.x, lastPoint.y, lastPoint.x, lastPoint.y)
                    self.chartFillDisplayWidth = lastPoint.x
                }
            }
...
//                
//                currentPoint = CGPointMake(CGFloat(cur.xIndex), CGFloat(cur.value) * phaseY)
//                trans.pointValueToPixel(&currentPoint)
//                
//                let xval =  (lineData.xVals[j]! as NSString).floatValue
//                
//                currentPoint.x = (CGFloat(xval) / lineData.maxXAxisValue) * self.viewPortHandler.contentWidth + self.viewPortHandler.offsetLeft
//
//                CGPathAddLineToPoint(cubicPath, nil, currentPoint.x, currentPoint.y);
//                
...
//            }
//            
//            self.chartFillDisplayWidth = currentPoint.x
        }
...
//        CGPathAddLineToPoint(spline, nil, pt2.x, pt2.y)
        ...
        // 渐变层对于Y轴负数不做分割处理
        CGPathAddLineToPoint(spline, nil, self.chartFillDisplayWidth, cubicHorizontalCuttingLinePosition());
        CGPathAddLineToPoint(spline, nil, pt2.x, cubicHorizontalCuttingLinePosition());
    ...
    private func cubicHorizontalCuttingLinePosition() -> CGFloat{
        return self.viewPortHandler.chartHeight - self.viewPortHandler.offsetBottom;
    }
    
//    private func pointFromDataEntry(dataEntry: ChartDataEntry?, transformer: ChartTransformer?) -> CGPoint{
//        
//        guard let tempDataEntry = dataEntry,
...
//        }
//
//        var point: CGPoint = CGPointMake(CGFloat((tempDataEntry.xIndex), tempDataEntry.value)
//        tempTransformer.pointValueToPixel(&point)
//        let xval =  (lineData.xVals[tempDataEntry.xIndex]! as NSString).floatValue
//        
//        point.x = (CGFloat(xval) / lineData.maxXAxisValue) * self.viewPortHandler.contentWidth + self.viewPortHandler.offsetLeft
//
//        return CGPointZero
//    }
    
    /// Draws the provided path in filled mode with the provided drawable.
    public override func drawFilledPath(context context: CGContext, path: CGPath, fill: ChartFill, fillAlpha: CGFloat)
    {
        CGContextSaveGState(context)
        CGContextBeginPath(context)
        ...
        rect.size.width = self.chartFillDisplayWidth
        
        fill.fillPath(context: context, rect: rect)
